...
- 穩定吞吐量可以使用奇偶效驗disk以外的其他Disk完成,因此提供 $(N-1) \cdot SMB/s$
- 寫入方面的話,RAID 4級使用full-stripe write的方式寫入
  - 假設要寫入block 0,1,2,3,先通過讀取的方式計算P0的新值(XOR(0,1,2,3)),接著就可以並行的方式寫入對應的請求的Block
    - 順序寫入的有效帶寬即為$(N-1) \cdot S MB/s$
- 隨機讀取的話帶寬為$(N-1) \cdot RMB/s$,因為隨機讀取將分布在系統的數據Disk上,奇偶效驗不會參與到
...
- 隨機寫入
  ...
  - RAID中小寫入性能非常糟糕,並且添加Disk也不會改善(原因是主要都是奇偶效驗Disk的性能瓶頸)為$(R/2) MB/s$
...
- 寫入Block 1和10的話,相當於
    1. 寫入Block 1轉變成對Disk 1和 Disk 4的請求(block 1和其奇偶效驗)
    2. 寫入Block 10轉變成對Disk 0和 Disk 2的請求(block 10和其奇偶效驗)
- 因此現在寫入的Disk都不一樣,因此可以並行寫入,而提高性能
- 因此帶寬為$ \frac{N}{4} \cdot RMB/s$,其中4被損失為每個RAID-5寫入產生的總IO操作
...
| ---------------- | ----------- | ---------------------------------------- | --------------------- | --------------------- |
| **Capacity**     | $N \cdot B$ | $\frac{N \cdot B}{2}$                    | $(N - 1) \cdot B$     | $(N - 1) \cdot B$     |
| **Reliability**  | 0           | 1 (for sure) or $\frac{N}{2}$ (if lucky) | 1                     | 1                     |
| **Throughput**   |             |                                          |                       |                       |
| Sequential Read  | $N \cdot S$ | $\frac{N}{2} \cdot S^1$                  | $(N - 1) \cdot S$     | $(N - 1) \cdot S$     |
| Sequential Write | $N \cdot S$ | $\frac{N}{2} \cdot S^1$                  | $(N - 1) \cdot S$     | $(N - 1) \cdot S$     |
| Random Read      | $N \cdot R$ | $N \cdot R$                              | $(N - 1) \cdot R$     | $N \cdot R$           |
| Rando Write     | $N \cdot R$ | $\frac{N}{2} \cdot R$                    | $\frac{1}{2} \cdot R$ | $\frac{N}{4} \cdot R$ |
| **Latency**      |             |                                          |                       |                       |
| Read             | $T$         | $T$                                      | $T$                   | $T$                   |
| Write            | $T$         | $T$                                      | $2T$                  | $2T$                  

1. 對性能不要求的話,strip顯然是最好的
2. 想要隨機IO的性能和可靠性,Mirror是最好的,但代價是容量下降
3. 容量和可靠性是主要目標,選擇RAID-5,付出的代價是小寫入的性能
4. 按順序執行IO操作並且希望容量最大化,選擇RAID-5也是最有意義的
